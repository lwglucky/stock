...
class camera:
    ...
    def keypress(this,key, x, y):
        if key in ('e', 'E'):
            this.move(0.,0.,3 * this.offest)
        if key in ('f', 'F'):
            this.move(3 * this.offest,0.,0.)
        if key in ('s', 'S'):
            this.move(-3 * this.offest,0.,0.)
        if key in ('d', 'D'):
            this.move(0.,0.,-3 * this.offest)
        if key in ('w', 'W'):
            this.move(0.,3 * this.offest,0.)
        if key in ('r', 'R'):
            this.move(0.,-3 * this.offest,0.)
        if key in ('v', 'V'):
        ...
        if key == GLUT_KEY_UP:
            this.offest = this.offest + 0.3
        if key == GLUT_KEY_DOWN:
            this.offest = this.offest - 0.3
    def mouse(this,x,y):  
        rx = (x - this.mouselocation[0]) * this.offest * 0.3
        ry = (y - this.mouselocation[1]) * this.offest * -0.3
        this.rotate(rx,ry)
        print x,y
